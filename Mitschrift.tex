...
\begin{definition}[Trennungseigenschaften]
	...
	\begin{enumerate}
		\item[T1] \label{T1} Falls $x \neq y \in X$ gibt es Umgebungen $\overbrace{U^x, U^y}^{\substack{\text{offene Mange}\\ \text{die } x \text{ enth"alt}}}$ mit $x \in U^x \setminus U^y, y \in U^y \setminus U^x$.
...
\begin{example}
	\begin{itemize}
		...
		\item Sei $X$ unendlich. Ist die koendliche Topologie $T$ Hausdorff? Nein. Ist sie T!? Ja: $U^x = X \setminus \{y\}, U^y = X \setminus \{x\}$
	\end{itemize}
\end{example}
\begin{remark}
...
\begin{example}
	Betrachte $\Pi : \begin{array}{c}
		...
		(x,y) \longmapsto x
	\end{array}$ mit euklidischer Topologie. $\Pi$ ist offen, aber nicht abgeschlossen: Betrachte $\overset{\text{abgeschlossen}}{x \cdot y = 1} \mapsto \overset{\substack{\text{nicht}\\ \text{abgeschlossen}}}{x \neq 0}$.
\end{example}
\begin{example}
	Sei $\begin{array}{c}
		X \longrightarrow Y\\
...
\end{definition}
\begin{definition}
	$(X,T)$ topologischer Raum. Die Menge $K \subset X$ ist kompakt, falls jede offene "Uberdeckung $K \subset \bigcup\limits_{i \in I} \underbrace{U_i}_{\text{offen}}$ eine endliche "Uberdeckung besitzt: Es gibt $i_1, \dots, i_n \in I$ mit $K \subset U_{i_1} \cup \dots \cup U_{i_n}$.\\
...
\section[Stoneraum]{Stoneraum von Typen einer Theorie}
